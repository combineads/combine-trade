# T-08-022 Strategy creation form
...
## Why
EP08 — The strategy list page (T-08-015) has a "New Strategy" button that navigates to `/strategies/new` but the page stub is empty. Users cannot create strategies without a form. This task provides the full create form using `react-hook-form` for validation.

## Inputs
- `packages/ui/src/views/strategies/strategy-list-view.tsx` (from T-08-015) — for navigation context
- `packages/ui/src/components/button.tsx`, `input.tsx`, `badge.tsx` (from T-08-012)
- `docs/DESIGN_SYSTEM.md` §5.1 (buttons), §5.3 (direction badges)
- `apps/web/src/app/(app)/strategies/new/page.tsx` — existing stub

## Dependencies
- T-08-015 (strategy list page — establishes ExecutionMode type and strategy views structure)
...
## Constraints
- Use `react-hook-form` for form state and validation (already in packages/ui dependencies per T-08-015)
...
- Code editor for strategy logic (T-08-016)
