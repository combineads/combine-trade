# T-12-005 Backtest page with trigger form and results
...
## Why
EP08 — The backtest feature is a core capability for validating strategies before enabling live trading. Without a UI, users cannot trigger backtests or review results. This task connects the result components from T-12-004 into a cohesive page with a trigger form.

## Inputs
- `packages/ui/src/views/backtest/` (all 5 result components from T-12-004)
- `packages/ui/src/components/button.tsx` (from T-08-012)
- `packages/ui/src/components/input.tsx` (from T-08-012)
- `docs/DESIGN_SYSTEM.md` §7.2 (empty states), §7.1 (loading skeletons)

## Dependencies
- T-12-004 (backtest result views)
