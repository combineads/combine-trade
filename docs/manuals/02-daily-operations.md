...
7. 경제 캘린더 스케줄러 시작 (고영향 경제 지표 발표 전후 거래 차단)
8. API 서버 시작 (웹 UI 제공)
9. SIGTERM/SIGINT 시그널 핸들러 등록
...
- [ ] 일일 손실 한도 미초과
- [ ] 경제 이벤트 거래 차단 현황 확인 (예정된 고영향 지표 발표 여부)
- [ ] DB 디스크 사용량 확인
...
| 단계 | 모드 | 기간 | 자본 | 조건 |
|------|------|------|------|------|
| 1 | — | — | — | 백테스트 기대값 > 0, WFO 효율성 > 0.5 |
| 2 | `analysis` | 2주 이상 | 30만원 | 신호만 관찰, 실제 주문 없음 |
| 3 | `alert` | 2주 이상 | — | Slack 알림으로 10건 이상 거래 검증 |
| 4 | `live` | — | 3천만원 | risk_pct 1%로 시작 |

각 단계를 충분히 검증한 후에만 다음 단계로 전환하세요.

> **EP-19 자동화**: 1단계 WFO 게이트(효율성 > 0.5)는 시스템이 자동으로 강제합니다.
> WFO를 통과하지 않으면 `live` 모드 전환이 차단됩니다. 수동 우회는 불가합니다.

> **1단계(백테스트/WFO)의 의미:**
> 단순 백테스트로 기대값이 양수인지 확인한 후, WFO로 과적합 여부를 검증합니다.
> WFO 효율성이 0.5 이상이면 전략이 새로운 데이터에서도 유효하다는 의미입니다.
> 상세 설명은 [백테스트 매뉴얼](./05-backtest.md)을 참고하세요.
