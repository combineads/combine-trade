# T-06-011 Wire execution worker to event bus
...
## Why
EP06 M4 — ExecutionWorkerHandler (T-06-008) needs to be wired to the event bus to receive decision events.

## Inputs
- `workers/execution-worker/src/handler.ts` (T-06-008 ExecutionWorkerHandler)
- Event bus module
- `db/schema/orders.ts` (orders table)

## Dependencies
- T-06-008 (execution worker handler)
...
- Start OrderStatusTracker (T-06-009) alongside event listener
