...
## Inputs
- T-06-001 `formatAlertMessage`, `AlertContext`, `SlackMessage` from `packages/alert`
- T-06-002 `getExecutionMode`, `isActionable`, `ExecutionModeDeps` from `packages/execution`
...
## Out of Scope
- Slack webhook client implementation (T-06-007)
- Event bus subscription setup (T-06-010)
- Alert DB schema and persistence implementation (injected dep)
- Order execution (T-06-008)
