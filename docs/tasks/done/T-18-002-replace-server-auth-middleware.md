# T-18-002 Replace server auth middleware
...
- `packages/shared/auth/better-auth.ts` — `auth` instance (output of T-18-001)
...
## Dependencies
- T-18-001 (better-auth instance must exist before it can be plugged in)
...
## Out of Scope
- Deleting legacy auth source files — T-18-003
- Session extraction in individual route handlers — T-18-006
- Client-side auth — T-18-007
- Rate limiting verification — T-18-008
