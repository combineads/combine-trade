# T-15-009 Rollback script with deploy-history.json tracking
...
## Inputs
- `scripts/deploy-history.json` (T-15-008 output) — source of the previous successful image tag
- `docker-compose.prod.yml` (T-15-007 output) — the compose file used for the image swap
...
## Dependencies
- T-15-008 (`scripts/deploy-history.json` format defined by deploy script; rollback.ts reads it)
...
## Out of Scope
- Deploy script — T-15-008
- Automatic rollback trigger during deploy — T-15-008 calls `rollback.ts` when post-deploy health fails (import relationship)
