# T-09-004 Risk gate (pre-order validation)
...
## Inputs
- T-09-001 `isBlocked`, `KillSwitchState` from `packages/core/risk/kill-switch.ts`
- T-09-002 `checkLimits`, `DailyLossConfig`, `LossTrackerDeps` from `packages/core/risk/loss-tracker.ts`
- T-09-003 `sizePosition`, `PositionSizeConfig`, `PositionSizeError` from `packages/core/risk/position-sizer.ts`
