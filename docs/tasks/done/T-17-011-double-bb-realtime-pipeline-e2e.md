# T-17-011 Double-BB realtime pipeline end-to-end verification

## Goal
Double-BB 전략이 실시간 파이프라인(candle close → strategy → vector → decision → alert)에서 정상 동작하는지 검증한다. p99 latency < 1초.

## Why
백테스트(T-17-010)가 완료되어도 실시간 파이프라인 연동이 검증되지 않으면 실제 트레이딩에 사용할 수 없다. 워커 프로세스 와이어링(EP21)이 완료된 후 최초 end-to-end 검증이 필요하다.

## Inputs
- T-17-010: 백테스트 완료 (벡터 + 라벨 존재)
- T-21-011: 모든 워커 프로세스 실행 가능 상태
...
## Steps
1. 기존 워커 코드 구조 파악 (T-21-011 완료 후)
...
## Out of Scope
Paper trading 실행 (T-17-012), live 배포 (T-17-013), 성능 최적화
