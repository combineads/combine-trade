...
## Task candidates
- T-104: Scaffold Elysia API server with IoC + AOP middleware
- T-105: Implement strategy CRUD API endpoints
- T-106: Implement candle/event/alert/order query endpoints
- T-107: Implement backtest trigger endpoint
- T-108: Implement execution mode change endpoint
- T-109: Implement SSE real-time streaming endpoint
- T-110: Scaffold Next.js web project with routing
- T-111: Implement Dashboard page (pipeline status overview)
- T-112: Implement Strategy list and detail pages
- T-113: Integrate Monaco Editor for strategy code editing
- T-114: Add TypeScript autocomplete for Strategy API in Monaco
- T-115: Implement Backtest UI (params → progress → results)
- T-116: Implement Events, Alerts, Orders pages
- T-117: Scaffold Tauri desktop project wrapping Next.js
- T-118: Add system tray, native notifications, auto-start
- T-119: Implement API client layer (Eden treaty or fetch wrapper)
...
- 2026-03-22: M3 tasks generated — T-128 (packages/ui scaffold), T-129 (apps/web scaffold), T-130 (base components), T-131 (API client hooks), T-132 (dashboard), T-133 (strategy list), T-134 (strategy editor), T-135 (monitoring pages), T-136 (risk management), T-137 (login/auth flow).
