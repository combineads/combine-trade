# T-040 Backtest labeler integration
...
## Inputs
- T-039 `BacktestEvent` type from `packages/backtest/types.ts`
- T-035 `labelEvent()` from `@combine/core/label` (signature: `labelEvent(input: LabelInput): LabelResult`)
- T-035 types: `LabelInput`, `LabelResult`, `CandleBar` from `@combine/core/label`
- Candle type from `@combine/candle`
- EP05 M2 spec (forward candle scan, immediate labeling)

## Dependencies
- T-039 (BacktestEvent and Candle[] context from replay engine)
- T-035 (labelEvent function and LabelInput/LabelResult types)
...
- slippage estimation statistics (backtest report, T-041)
