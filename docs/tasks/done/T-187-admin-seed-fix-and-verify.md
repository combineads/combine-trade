# T-187 admin-seed-fix-and-verify

## Goal
`db/seed/admin.ts`의 dynamic import 경로를 수정하고, `bun run db:seed:admin` 실행 후 실제 로그인이 성공함을 확인한다.

## Why
EP18 T-183에서 admin seed 스크립트가 작성됐으나 `../index.js` import 경로가 맞지 않아 실패했을 수 있다. Auth wiring(T-186) 후 실제 로그인 검증이 필요하다.

## Inputs
- T-186 완료 (auth wiring 완료)
- `db/seed/admin.ts` (EP18 T-183 산출물)
