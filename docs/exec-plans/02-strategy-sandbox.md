...
## Task candidates
- T-02-001: Implement SMA, EMA, WMA, BB, RSI, MACD indicators with tests
- T-02-002: Implement ATR, Stochastic, CCI, ADX indicators with tests
- T-02-003: Implement OBV, VWAP indicators with tests
- T-02-004: Create strategies DrizzleORM schema and CRUD service + strategy version management (immutable versions)
- T-02-005: Implement strategy code pre-save validation (TypeScript parsing, forbidden API detection) + Strategy API type compatibility check at save time
- T-02-006: Build sandbox runtime with V8 isolates (isolated-vm) + sandbox resource limits (128MB memory, 500ms timeout) + kill switch integration (ERR_FATAL_SANDBOX_OOM/TIMEOUT)
- T-02-007: Design and implement Strategy API (candle access, indicators) + defineFeature API for vector feature registration + optional setEntry/setExit custom trade conditions
- T-02-008: Implement multi-timeframe data access and warm-up period auto-detection in sandbox API
- T-02-013: Implement warm-up period event suppression (real-time + backtest)
- T-02-010: Build strategy-worker with LISTEN candle_closed → evaluate + per-strategy error isolation
...
- All tasks T-02-001 through T-02-013 completed and moved to docs/tasks/done/.
