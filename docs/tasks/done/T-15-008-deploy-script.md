# T-15-008 Deploy script with pre-flight checks and graceful shutdown
...
## Inputs
- `docker-compose.prod.yml` (T-15-007 output) — the compose file used for the swap
...
- `db/schema/kill-switch.ts` (T-09-001) — kill_switch_state table schema
- `db/schema/orders.ts` (T-00-003) — orders table schema for open order check
- `GET /api/health` — health endpoint for pre-deploy and post-deploy verification

## Dependencies
- T-15-007 (`docker-compose.prod.yml` must exist — deploy script calls `docker compose -f docker-compose.prod.yml up -d`)
...
## Out of Scope
- Rollback script — T-15-009 (deploy script calls rollback on post-deploy failure, but rollback.ts is implemented in T-15-009)
- Staging deploy and promotion gate — T-15-010 (release workflow)
...
- **Rollback on post-deploy failure**: Script prints the rollback command but delegates to T-15-009 (`scripts/rollback.ts`) as specified in Out of Scope.
