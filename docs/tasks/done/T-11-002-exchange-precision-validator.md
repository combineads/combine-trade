...
## Goal
Implement pure precision validation and rounding functions in `packages/shared/decimal/precision.ts` that enforce exchange-specific rules for price tick size, quantity lot size, and minimum notional value. All functions use Decimal.js arithmetic (via the T-11-001 wrapper) and are stateless — they receive precision rules as input parameters, not from a database or exchange API.
...
- T-11-001 `packages/shared/decimal/arithmetic.ts` — `mul`, `div`, `round`, `sub`, `RoundingMode`
- EP11 decision: rounding mode for orders = `ROUND_DOWN` (truncate)

## Dependencies
None. (T-11-001 is in the same package and will exist at implementation time, but T-11-002 can be implemented independently since the arithmetic wrapper interface is defined.)
...
## Constraints
- All arithmetic must use the T-11-001 Decimal.js wrapper — no direct `new Decimal()` calls
...
- All functions use Decimal.js via the T-11-001 arithmetic wrapper
