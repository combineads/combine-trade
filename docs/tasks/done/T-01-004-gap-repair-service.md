# T-01-004 Implement gap detection and REST-based gap repair service
...
- `workers/candle-collector/repository.ts` — DrizzleCandleRepository with upsertBatch, findLatestOpenTime (T-01-002)
...
- T-00-005 outputs: validateContinuity, isContinuous
- T-01-001 outputs: BinanceAdapter.fetchOHLCV
- T-01-002 outputs: DrizzleCandleRepository.upsertBatch, findLatestOpenTime

## Dependencies
- T-00-005 (continuity validation — validateContinuity, isContinuous)
- T-01-001 (BinanceAdapter.fetchOHLCV — used for REST backfill)
- T-01-002 (DrizzleCandleRepository.upsertBatch, findLatestOpenTime)
...
## Constraints
- Gap repair operates on at-most 1-day window via REST (longer gaps → Vision archive, handled in T-01-005 startup recovery)
...
## Out of Scope
- Binance Vision daily archive backfill (Tier 2 — handled in T-01-005 startup recovery logic)
