# T-175 Release workflow + conventional commit PR lint
...
## Inputs
- `.github/workflows/ci.yml` (T-166 output) — the CI workflow that release.yml re-runs
- `.github/workflows/build.yml` (T-172 output) — image build/push workflow that release.yml extends
...
- `.harness/benchmarks/baseline.json` (T-167 output) — updated on each production deploy

## Dependencies
- T-166 (`.github/workflows/ci.yml` must exist — release workflow calls it or re-runs equivalent jobs)
