...
## Why
Correct position sizing is fundamental to risk control. The fixed-fraction model ensures that each trade risks only a fixed percentage of account balance, limiting drawdown per trade regardless of price level. Making this pure and injectable means the risk gate (T-09-004) can call it without any exchange connectivity, and it can be tested with arbitrary balance / price scenarios.
...
- `packages/core/risk/types.ts` (T-09-001/T-09-002) — add `PositionSizeConfig`, `PositionSizeResult`, `PositionSizeError`
