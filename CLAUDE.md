...
| Epic Planner | `harness-epic-planner` | 2 | Break work into epic-level execution plans |
| Task Generator | `harness-task-generator` | 3 | Seed the backlog with execution-ready tasks |
| Implementer | `harness-implementer` | 3.1 | Pick tasks, write code, run validation |
| Reviewer | `harness-reviewer` | 3.2 | Review implementations against 8-item checklist |
| Task Closer | `harness-task-closer` | 4 | Close tasks, update docs, promote rules |
| Cleanup | `harness-cleanup` | 5 | Scan for drift, prune stale work, update scores |
| Orchestrator | `harness-orchestrator` | — | (optional) Wraps Stages 3.1–3.2 for parallel task execution |

## Development workflow (TDD: Red-Green-Refactor)

```
project-bootstrap → epic-planner → task-generator → implementer → reviewer → task-closer → harness-cleanup
     Stage 1          Stage 2         Stage 3        Stage 3.1     Stage 3.2     Stage 4        Stage 5
   "what to build"  "what chunks"   "what to do"   "RED-GREEN   "verify quality" "feed back"  "clean up"
                                                     -REFACTOR"
```

The build loop runs Stages 3–4:
- **task-generator** seeds the backlog
- **implementer** follows TDD: RED (failing tests) → GREEN (minimal implementation) → REFACTOR (clean code)
- **reviewer** evaluates quality and TDD compliance
- **task-closer** closes approved tasks and reports newly unblocked work

Stage 5 (cleanup) runs periodically outside the build loop to scan for drift, prune stale work, and update scores.
...
/harness-orchestrator T-005 T-006         # parallel feature development
/harness-orchestrator --review T-005      # parallel multi-role review
```
