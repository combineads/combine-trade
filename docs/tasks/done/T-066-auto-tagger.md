...
- T-065 `packages/core/journal/assembler.ts` — `TradeJournal` type
- T-064 `packages/core/journal/market-context.ts` — `MarketContext`, `TrendContext`, `TrendDirection`
- T-063 `packages/core/journal/entry-snapshot.ts` — `EntrySnapshot`, `SnapshotDecision`
- Architecture guardrail: `packages/core/journal/` must not import Elysia, CCXT, Drizzle, or Slack

## Dependencies
- T-065 (journal assembler) — provides `TradeJournal` type that the tagger consumes
...
## Constraints
- All ratio comparisons use the T-058 Decimal.js wrapper — no native float for string-typed values
