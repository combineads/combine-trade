# T-08-024 Top bar kill switch indicator
...
## Why
EP08 — The kill switch state must be visible in the persistent top navigation bar so that operators are always aware of the system's safety state, regardless of which page they are on. The `NotificationBanner` (T-08-023) handles page-level alerting; the `TopBar` indicator provides a compact persistent indicator within the navigation chrome.

## Inputs
- `packages/ui/src/components/top-bar.tsx` — existing TopBar component (from T-08-010/T-08-011)
- `packages/ui/src/components/notification-banner.tsx` — KillSwitchBanner (from T-08-023)
- `docs/DESIGN_SYSTEM.md` §5.4 (card kill-switch state: `--color-danger` red)
- `docs/DESIGN_SYSTEM.md` §3.1 (top bar layout)

## Dependencies
- T-08-023 (NotificationBanner — establishes kill switch visual vocabulary)
