# T-057 Risk management integration test
...
## Why
Unit tests for T-053 through T-056 verify each module in isolation. This integration test catches wiring bugs and interface mismatches that only surface when all four risk modules are composed through the gate. It also serves as a living specification for how the risk gate is expected to be wired up in the order executor, making the dependency flow explicit and machine-verifiable before the executor integration is built.

## Inputs
- T-053 `activate`, `isBlocked`, `KillSwitchState`, `KillSwitchDeps` from `packages/core/risk`
- T-054 `checkLimits`, `LossTrackerDeps`, `DailyLossConfig` from `packages/core/risk`
- T-055 `sizePosition`, `PositionSizeConfig` from `packages/core/risk`
- T-056 `validateOrder`, `RiskGateDeps`, `OrderValidationInput`, `GateResult` from `packages/core/risk`
