...
export const killSwitchState = pgTable(
	"kill_switch_state",
	{
	...
	(table) => [index("kill_switch_state_user_id_idx").on(table.userId)],
);
...
export const killSwitchEvents = pgTable(
	...
	(table) => [index("kill_switch_events_user_id_idx").on(table.userId)],
);
