# T-172 docker-compose.prod.yml + .dockerignore
...
## Inputs
- `Dockerfile.api` (T-169 output)
- `Dockerfile.workers` (T-170 output)
- `Dockerfile.web` (T-171 output)
...
- `.github/workflows/ci.yml` (T-166 output) — CI must pass before build workflow can reference a green commit

## Dependencies
- T-169 (`Dockerfile.api` must exist)
- T-170 (`Dockerfile.workers` must exist)
- T-171 (`Dockerfile.web` must exist)
...
# Test bring-up with local images (requires T-169, T-170, T-171 built locally)
...
## Out of Scope
- Deploy script (pre-flight checks, graceful shutdown, swap logic) — T-173
- Rollback script — T-174
