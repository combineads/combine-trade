# T-17-008 Double-BB strategy seed registration script
...
## Inputs
- `packages/core/strategy/double-bb/script.ts` (T-17-006: strategy script)
...
- Date: 2026-03-22
- Files changed: `packages/core/strategy/double-bb/config.ts`, `packages/core/strategy/double-bb/__tests__/config.test.ts`, `index.ts` updated
- Approach: Created DoubleBBStrategyConfig type and buildDoubleBBConfig(direction) factory function
- Config includes: indicator_config (BB20+BB4+MAs+ATR), features_definition (10 features), search/result/decision configs
- threshold = √10 × 0.3 ≈ 0.949 per EP03 formula
- Actual DB seed script deferred — requires DB connection; config module is the testable foundation
- Tests: 10 tests covering both directions, feature defs, indicator config, search/result/decision configs
- Validation: 10/10 pass, typecheck clean

## Outputs
- `buildDoubleBBConfig(direction)` → `DoubleBBStrategyConfig`
- `DOUBLE_BB_FEATURES_DEFINITION`, `DOUBLE_BB_INDICATOR_CONFIG`, `DOUBLE_BB_TIMEFRAMES` constants
- All exports added to barrel `index.ts`
