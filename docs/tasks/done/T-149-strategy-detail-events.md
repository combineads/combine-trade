# T-149 Strategy detail event list tab
...
## Why
EP08 — The strategy detail page (T-134) has a tab navigation structure. The events tab is the primary analytical view: it shows which LONG/SHORT signals the strategy fired, whether they won or lost, and their timestamps. This data is the evidence traders use to evaluate strategy quality.

## Inputs
- `packages/ui/src/components/data-table.tsx` (from T-135)
- `packages/ui/src/components/badge.tsx` — DirectionBadge, StatusBadge (from T-130)
- `packages/ui/src/components/pagination.tsx` (from T-130/T-135)
- `docs/DESIGN_SYSTEM.md` §5.5 (table), §5.3 (direction badges), §7.2 (empty state)

## Dependencies
- T-135 (monitoring pages — DataTable and Pagination components)
...
## Constraints
- Use existing `DataTable<T>` generic component from T-135 — do not re-implement a table
...
## Out of Scope
- SSE real-time event streaming into the table (T-148 integration)
