...
- Deliverables:
  ...
  - ExchangeAdapter REST 메서드 구현:
    - `fetchBalance()`, `fetchPositions()`, `fetchOrders()`
    - `createOrder()`, `cancelOrder()`, `editOrder()`
    - `setSL()`, `setLeverage()`
  - WS 메서드(`subscribeKlines()`, `unsubscribeKlines()`)는 stub — M2에서 구현
- Acceptance criteria:
  - Binance Futures 테스트넷에서 주문 생성/취소 성공
  - 레이트리밋 (1200 req/min) 준수
  - 에러 시 지수 백오프 (1s→2s→4s, max 30s)
  - CCXT 응답의 number를 base.ts에서 Decimal로 변환 — ExchangeAdapter 반환값의 모든 price/size는 Decimal 타입
  - createOrder()가 idempotency_key를 생성하여 반환 (재시도 시 중복 주문 방지)
- Validation:
...
- Deliverables:
  ...
  - BinanceAdapter.subscribeKlines()/unsubscribeKlines() 구현 (M1 stub → 실제 WS 연결)
  - Binance WebSocket kline 스트림 구독/해제
  - 연결 끊김 감지 → 재연결 (1s→2s→4s, max 30s) — ARCHITECTURE.md 에러 처리 전략 준수
- Acceptance criteria:
  - 다중 심볼×타임프레임 동시 구독
  - 30초 이내 자동 재연결
...
- T-03-001: exchanges/base.ts — CCXT 공통 래퍼 (인증, 레이트리밋, 에러 매핑, Decimal 변환) + ports.ts setLeverage 추가
- T-03-002: exchanges/binance.ts — BinanceAdapter 읽기 작업 (fetchBalance, fetchPositions, fetchOrder, fetchOHLCV, getExchangeInfo)
- T-03-003: exchanges/binance.ts — 주문 생성/취소/수정 (createOrder, cancelOrder, editOrder, idempotency_key)
- T-03-004: exchanges/binance.ts — SL 등록(stop_market) & 레버리지 설정 + CreateOrderParams.type 확장
- T-03-005: exchanges/ws-manager.ts — WebSocket 연결 관리자 (자동 재연결, 지수 백오프)
- T-03-006: exchanges/binance.ts — Binance WebSocket kline 구독 (watchOHLCV 구현)
- T-03-007: exchanges/okx.ts — OKX 어댑터 스캐폴드
- T-03-008: exchanges/bitget.ts — Bitget 어댑터 스캐폴드
- T-03-009: exchanges/mexc.ts — MEXC 어댑터 스캐폴드 + ADR-005 (2-step SL 설계)
- T-03-010: 거래소 어댑터 통합 테스트 (mock CCXT) + exchanges/index.ts 팩토리
...
## Progress notes
- 2026-04-04: 태스크 생성 완료 (10개). ports.ts 인터페이스와 에픽 계획 차이점 반영 — setLeverage 추가, stop_market 타입 확장, watchOHLCV(epic의 subscribeKlines) 매핑.
- 2026-04-04: EP-03 전체 구현 완료. 10/10 태스크 done. 6 Wave (WIP=2) 병렬 실행. 743 tests, 0 fail, typecheck/lint clean.
