# T-041 Backtest statistics report
...
## Inputs
- T-040 `LabelResult` type from `@combine/core/label` (fields: `resultType`, `pnlPct`, `holdBars`, `exitPrice`, `slHitFirst`)
- T-039 `BacktestEvent` type from `packages/backtest/types.ts` (fields: `openTime`, `direction`, `entryPrice`, `candleIndex`)
...
## Dependencies
- T-040 (LabelResult available from labeler integration — defines the labeled event shape)
