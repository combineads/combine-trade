# T-10-009 Implement exchange credential encryption (AES-256-GCM)
...
- **Date**: 2026-03-22
- **Files changed**: `packages/shared/auth/encryption.ts` (new), `packages/shared/auth/__tests__/encryption.test.ts` (new — 16 tests)
- **Approach**: TDD. AES-256-GCM via Node.js crypto. Random 12-byte IV per encryption. Master key from 64-char hex string. maskApiKey shows first 3 + "****" + last 4 chars, keys <7 chars fully masked. CredentialService wraps encrypt/decrypt with DI-injected DB ops.
- **Validation**: 16/16 tests pass, typecheck clean, 1057 total tests.

## Outputs
- `encrypt(plaintext, masterKeyHex)` → `EncryptedData { ciphertext, iv, tag }`
- `decrypt(ciphertextHex, ivHex, tagHex, masterKeyHex)` → plaintext
- `maskApiKey(key)` → masked string
- `CredentialService` class with `save()`, `get()`, `delete()`, `list()` methods
- `CredentialServiceDeps`, `StoredCredential`, `CredentialListItem` interfaces
