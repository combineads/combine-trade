...
## Why
The current web client has no built-in session management tied to better-auth. After T-18-002 the server issues better-auth sessions (httpOnly cookies). The web client must use the matching better-auth React client to sign in, read session state, and sign out. The shared `useAuth` hook in `packages/ui/` ensures that both `apps/web/` and `apps/desktop/` speak the same auth abstraction.
...
## Dependencies
- T-18-002 (better-auth server endpoints must exist before the client can connect to them)
...
Note: the cookie name `combine-trade.session_token` must match the `cookiePrefix` set in `better-auth.ts` (T-18-001). Verify the exact cookie name from better-auth docs.
...
- Tauri desktop client is out of scope for this task — T-18-007 covers web only
