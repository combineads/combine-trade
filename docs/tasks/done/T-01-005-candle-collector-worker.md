# T-01-005 Implement candle-collector worker (WebSocket + startup recovery + NOTIFY)
...
## Inputs
- `workers/candle-collector/repository.ts` — DrizzleCandleRepository (T-01-002)
- `workers/candle-collector/gap-repair.ts` — GapRepairService (T-01-004)
- `packages/exchange/binance/adapter.ts` — BinanceAdapter with watchOHLCV (T-01-001)
- `packages/shared/event-bus/publisher.ts` — PgEventPublisher (T-00-009)
- `packages/shared/event-bus/channels.ts` — Channels.candleClosed (T-00-009)
- `packages/shared/di/container.ts` — IoC container (T-00-006)
...
- T-00-009 outputs: PgEventPublisher, Channels.candleClosed
- T-01-001 outputs: BinanceAdapter.fetchOHLCV, BinanceAdapter.watchOHLCV
- T-01-002 outputs: DrizzleCandleRepository.upsertBatch, findLatestOpenTime
- T-01-004 outputs: GapRepairService.repairAll

## Dependencies
- T-01-001 (BinanceAdapter with watchOHLCV)
- T-01-002 (DrizzleCandleRepository with upsertBatch + findLatestOpenTime)
- T-01-004 (GapRepairService)
- T-00-009 (PgEventPublisher — event bus)
