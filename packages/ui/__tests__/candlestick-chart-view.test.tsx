...
import { TimeframeSelector, type Timeframe } from "../src/views/charts/timeframe-selector";
...
describe("TimeframeSelector", () => {
	test("renders 7 timeframe buttons", () => {
		const html = renderToString(
			<TimeframeSelector current="1h" onSelect={() => {}} />,
		);
	...
	test("highlights active timeframe", () => {
		const html = renderToString(
			<TimeframeSelector current="4h" onSelect={() => {}} />,
		);
...
describe("SymbolSelector", () => {
	test("renders options for each symbol", () => {
		const html = renderToString(
			<SymbolSelector symbols={["BTC/USDT", "ETH/USDT", "SOL/USDT"]} value="BTC/USDT" onChange={() => {}} />,
		);
...
describe("CandlestickChartView", () => {
	test("renders symbol name", () => {
		const html = renderToString(
			<CandlestickChartView symbol="BTC/USDT" timeframe="1h" />,
		);
		expect(html).toContain("BTC/USDT");
	});
	...
	test("renders timeframe selector", () => {
		const html = renderToString(
			<CandlestickChartView symbol="BTC/USDT" timeframe="4h" />,
		);
		expect(html).toContain("4h");
	});
