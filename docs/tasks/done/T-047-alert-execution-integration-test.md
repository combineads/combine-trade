# T-047 Alert-execution integration test
...
## Why
Unit tests for T-043 through T-046 verify each module in isolation. This integration test catches wiring bugs, incorrect mode guards, and interface mismatches that only surface when all modules are composed. It also serves as a living specification of the expected behaviour for each execution mode, making mode semantics explicit and machine-verifiable.

## Inputs
- T-043 `formatAlertMessage`, `AlertContext` from `packages/alert`
- T-044 `getExecutionMode`, `setExecutionMode`, `isActionable`, `requiresOrder`, `ExecutionModeDeps` from `packages/execution`
- T-045 `buildOrder`, `generateClientOrderId`, `OrderContext` from `packages/execution`
- T-046 `handleDecisionCompleted`, `AlertWorkerDeps`, `DecisionCompletedEvent` from `workers/alert-worker/src/handler`
- `DecisionResult` from `@combine/core/decision`
- T-037 `tests/integration/label-decision.test.ts` as structural reference
...
- Execution worker handler (separate task)
- Paper trading simulation
- Retry exhaustion scenario (covered in T-046 unit tests)
