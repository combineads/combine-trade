# T-207 Double-BB backtest execution & stats validation

## Goal
Double-BB LONG/SHORT 전략에 대해 3년치 백테스트를 실행하고, 통계(trades ≥ 100, expectancy > 0)를 검증한다. 완료 후 HNSW REINDEX를 수행한다.

## Why
전략 코드(T-096~T-102)와 seed 스크립트(T-102)는 완료됐지만, 실제 3년치 데이터로 백테스트를 돌려 벡터+라벨을 생성하고 통계를 검증하지 않았다. 이 단계 없이 실시간 파이프라인에서 kNN 검색이 불가능하다(cold start).

## Inputs
- T-206 출력물: 3yr BTCUSDT 1m 캔들 DB에 적재 완료
- T-102 출력물: Double-BB LONG/SHORT 전략 DB 등록 완료 (strategy_id 필요)
- `packages/backtest/engine.ts` — 백테스트 엔진 (T-039 결과물)
- `packages/backtest/report.ts` — 통계 리포트 (T-041 결과물)
...
## Out of Scope
실시간 파이프라인 검증 (T-208), 파라미터 최적화, Walk-forward 분석
