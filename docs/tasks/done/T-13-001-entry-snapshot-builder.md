...
## Why
EP13 M1 requires capturing the full decision context at the moment a LONG/SHORT signal fires. Without a structured snapshot, the journal assembler (T-13-003) cannot reconstruct why a trade was entered. By implementing this as a pure function that takes already-resolved inputs, the snapshot builder is testable in isolation, has zero IO dependencies, and can be called from both the live pipeline and the backtest engine. The snapshot preserves decision statistics, matched pattern details, and the raw + normalized feature vector for post-trade analysis.
...
- Market context enrichment (covered by T-13-002)
