# T-140 Candlestick chart view with controls
...
## Why
EP08 — Dashboard and strategy detail pages require a candlestick chart. This task establishes the layout, control components (TimeframeSelector, SymbolSelector), and the view composition so that later chart-library integration (T-141+) only needs to swap the placeholder for a real chart instance.

## Inputs
- `packages/ui/src/components/chart-container.tsx` (from T-139)
