# VECTOR_SPEC — 202-Dimensional Feature Vector

Specification for the KNN feature vector used in signal validation.

- Source: `src/vectors/features.ts`
- Dimension: 202
- Normalization: Median / IQR per feature (computed during WFO training)
- Timeframes: 5M (primary), 1M (secondary)
...
|----------------|-------|-----------|
| price_position | 40    | 0–39      |
| momentum       | 30    | 40–69     |
| volatility     | 30    | 70–99     |
| trend          | 40    | 100–139   |
| time_series    | 50    | 140–189   |
| strategy       | 12    | 190–201   |
| **TOTAL**      | **202** |         |
...
Features not listed have a default weight of **1.0**.

| Feature               | Weight | Rationale                                    |
|-----------------------|--------|----------------------------------------------|
| `bb4_position`        | 2.0    | Primary entry trigger; highest signal value  |
| `pivot_distance`      | 1.5    | S/R proximity strongly affects trade outcome |
| `daily_open_distance` | 1.5    | Intraday bias reference point                |
| `session_box_position`| 1.5    | Session range context for entry quality      |

---

## Category: price_position (40)

### 5M Bollinger and MA price position (17, indices 0–16)
...
### 1M Bollinger and MA price position (17, indices 17–33)
...
### Cross-timeframe price position (6, indices 34–39)
...
### Cross-timeframe and composite momentum (12, indices 58–69)
...
| `rsi14_slope_5m` | `(rsi14_5m[0] - rsi14_5m[1]) / 100` [5M] |
| `rsi14_slope_1m` | `(rsi14_1m[0] - rsi14_1m[1]) / 100` [1M] |
| `price_vs_bb20mid_velocity_5m` | `close_vs_bb20mid_5m[0] - close_vs_bb20mid_5m[1]` [5M] |
| `price_vs_bb20mid_velocity_1m` | `close_vs_bb20mid_1m[0] - close_vs_bb20mid_1m[1]` [1M] |
...
### 5M volatility (13, indices 70–82)
...
### 1M volatility (13, indices 83–95)

| Name | Formula |
|------|---------|
| `atr14_1m` | `atr14_1m / close` [1M] |
| `atr14_norm_1m` | `atr14_1m / bb20_bandwidth_abs` [1M] |
...
### Cross-timeframe volatility (4, indices 96–99)
...
### Cross-timeframe trend (14, indices 126–139)
...
### 5M lagged and rolling features (22, indices 140–161)
...
### 1M lagged and rolling features (22, indices 162–183)
...
### Cross time-series derived (6, indices 184–189)

| Name | Formula |
|------|---------|
| `bb20_pct_b_lag1_ratio` | `bb20_pct_b_lag1_5m - bb20_pct_b_lag1_1m` |
| `rsi14_lag1_ratio` | `rsi14_lag1_5m - rsi14_lag1_1m` |
| `pattern_123_5m` | `sum(sign(r1), sign(r2), sign(r3))` — 3-bar return direction [5M] |
| `pattern_123_1m` | 3-bar return direction pattern [1M] |
| `bb4_sequence_5m` | `mean(diff(bb4_pct_b[0..3]))` — bb4 pct_b trend over 3 bars [5M] |
| `bb4_sequence_1m` | bb4 pct_b trend over 3 bars [1M] |

---

## Category: strategy (12, indices 190–201)

Double-BB strategy-specific derived features capturing entry signal context.

| Name | Formula | Weight |
|------|---------|--------|
| `bb20_position` | `(bb20_pct_b_5m * 2) - 1` — BB20 %b mapped to `[-1,1]` | 1.0 |
| `bb4_position` | `(bb4_pct_b_5m * 2) - 1` — BB4 %b mapped to `[-1,1]` | **2.0** |
| `ma_ordering` | Ordinal MA stack: `sma20 vs sma60 vs sma120` encoded `[-1,0,1]` | 1.0 |
| `ma20_slope` | `(sma20[0] - sma20[3]) / sma20[3]` — 3-bar slope of sma20 [5M] | 1.0 |
| `atr_separation` | `(bb4_upper - bb4_lower) / atr14` — band width in ATR units [5M] | 1.0 |
| `pivot_distance` | `(close - nearest_pivot) / atr14` — distance to nearest S/R pivot | **1.5** |
| `rsi_normalized` | `(rsi14_5m - 50) / 50` — RSI centered and scaled to `[-1,1]` | 1.0 |
| `rsi_extreme_count` | count of bars in last 5 where `rsi14 > 70` or `rsi14 < 30` (range 0–5) | 1.0 |
| `breakout_intensity` | `(close - bb20_upper) / atr14` if above upper; `(bb20_lower - close) / atr14` if below lower; else `0` | 1.0 |
| `disparity_divergence` | `bb4_pct_b_5m - bb20_pct_b_5m` — inner vs outer band divergence | 1.0 |
| `daily_open_distance` | `(close - daily_open) / atr14` — distance from daily open | **1.5** |
| `session_box_position` | `(close - session_low) / (session_high - session_low)` mapped to `[-1,1]` | **1.5** |

---

## Design Notes

- `bb4_position` weight 2.0 — this is the primary entry trigger in the Double-BB strategy; highest per-feature discriminative value in backtests.
- `pivot_distance`, `daily_open_distance`, `session_box_position` weight 1.5 — price context anchors that strongly correlate with whether an entry near a BB level succeeds or fails.
- All features are normalized post-extraction using Median/IQR computed per WFO fold; the formulas above describe raw (pre-normalization) values.
- `nearest_pivot` is the closest swing high or swing low from the last 20 bars.
- `session_high`/`session_low` are the intraday high/low since 00:00 UTC of the current trading session.
- `daily_open` is the first `open` price of the UTC calendar day.
