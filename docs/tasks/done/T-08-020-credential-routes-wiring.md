# T-08-020 Wire credential routes into API server

## Goal
`apps/api/src/server.ts`에 credential 라우트를 연결하고, DB 기반 Drizzle repository를 의존성으로 주입한다.

## Why
T-08-008(API server bootstrap)에서 credential 라우트 wiring이 누락됐다.
...
## Dependencies
- T-08-008 (API server bootstrap — server.ts 존재)
- T-10-003 (AES-256-GCM 암호화 서비스)
- T-10-004 (credentialRoutes 팩토리)
