# T-10-002 Auth middleware + login/refresh API
...
## Inputs
- T-08-001: `ApiError`, `UnauthorizedError`, `ok` helpers
- T-10-001: `signAccessToken`, `signRefreshToken`, `verifyToken`, `hashPassword`, `comparePassword`, `JwtPayload`, `TokenPair`
...
- `apps/api/src/middleware/auth.ts`
  - `authPlugin` — Elysia plugin that:
    ...
    - Calls `verifyToken` from T-10-001
...
- Applying `authPlugin` to existing routes T-08-002 through T-08-005 (separate task after all routes exist — see T-10-004)
