...
## Inputs
- `.github/workflows/ci.yml` (T-15-001 output) — workflow to extend
...
## Dependencies
- T-15-001 (`.github/workflows/ci.yml` must exist before adding security jobs)
...
## Out of Scope
- Sandbox escape test job — sandbox escape tests belong to T-02-006 (sandbox-runtime). The CI job to run them can be added to ci.yml in that epic if needed.
...
- Container image vulnerability scanning (Trivy/Snyk) — deferred until Docker images exist (T-15-004+)
