...
export function TradeStats({ stats }: TradeStatsProps) {
	return (
		<div style={{
			...
			<Stat label="Win Rate" value={`${(stats.winrate * 100).toFixed(1)}%`} color={stats.winrate > 0.5 ? "var(--color-win)" : "var(--text-secondary)"} />
			<Stat label="Expectancy" value={stats.expectancy.toFixed(2)} color={stats.expectancy > 0 ? "var(--color-win)" : "var(--color-secondary)"} />
			<Stat label="Profit Factor" value={stats.profitFactor.toFixed(2)} />
			<Stat label="Max Drawdown" value={`${stats.maxDrawdown.toFixed(1)}%`} color="var(--color-secondary)" />
