# T-169 Dockerfile.api — multi-stage Bun image for Elysia API
...
## Why
Containerising the API server is required for the production deployment model described in EP15. Without a Dockerfile, the deploy script (T-173) cannot pull a versioned image and swap it in. Multi-stage builds keep image size small and ensure dev dependencies and source maps never reach production.
...
- `.dockerignore` (or confirm it covers api-relevant exclusions — shared with T-172)
...
- GitHub Actions build workflow (`.github/workflows/build.yml`) — T-172
- Image registry push — T-172
